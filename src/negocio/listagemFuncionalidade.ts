...
export default class ListagensFuncionalidades extends Listagem{
    ...
    public listagem10ConsumoQnt(): void {
        ...
        let opcaoListTop10 = this.entrada.receberNumero('Insira uma opcao para comparar o consumo: ')
        switch(opcaoListTop10){
            case 1:
                let clienteOrdenadosProd = this.clientes.map(cliente => {
                    return {
                        clienteNome: cliente.nome,
                        clienteCpf: cliente.getCPF.getValor,
                        totalProdConsumido: cliente.getQntProdConsumido
                    }
                }).sort((a, b) => b.totalProdConsumido - a.totalProdConsumido).slice(0, 10)
                console.log(`\n=================Lista Top 10 - Produtos=================`)
                let rankProd = 0
                if(clienteOrdenadosProd.length == 0){
                    ...
                    clienteOrdenadosProd.forEach(cliente => {
                        ...
                        console.log(`   Quantidade de Produtos: ${cliente.totalProdConsumido}`)
                    })
                }
                break
            case 2:
                let clienteOrdenadosServ = this.clientes.map(cliente => {
                    return {
                        clienteNome: cliente.nome,
                        clienteCpf: cliente.getCPF.getValor,
                        totalServConsumido: cliente.getQntServConsumido
                    }
                }).sort((a, b) => b.totalServConsumido - a.totalServConsumido).slice(0, 10)
                console.log(`\n=================Lista Top 10 - Servicos=================`)
                let rankServ = 0
                if(clienteOrdenadosServ.length == 0){
                    ...
                    clienteOrdenadosServ.forEach(cliente => {
                        ...
                        console.log(`   CPF: ${cliente.clienteCpf}`)
                        console.log(`   Quantidade de Servicos: ${cliente.totalServConsumido}`)
                    })
                }
                break
            case 0:
    ...
    public listagemGenero(): void{
        ...
        switch(opcaoGenero){
            case 1:
                let cliGeneroMasc = this.clientes.filter(cliente => cliente.genero == "M")
                console.log(`\n=================Lista Masculina=================\n`)
                if (cliGeneroMasc.length == 0){
                    console.log("Nenhum Cliente do genero Masculino foi cadastrado ainda")
                }else{
                    cliGeneroMasc.forEach(cliente => {
                        console.log(`Nome: ${cliente.nome}`)
                        console.log(`CPF: ${cliente.getCPF.getValor}`)
                        console.log(`Genêro: ${cliente.genero}`)
                        console.log(`Total de Produtos Consumido: ${cliente.getQntProdConsumido}`)
                        console.log(`Total de Servicos Consumido: ${cliente.getQntServConsumido}\n`)
                    })
                }
                break
            case 2:
        ...
        switch(opcaoMaisConsumidos){
            case 1:
                if(this.produtos.length == 0) {
                    ...
                    let prodMaisConsumido = this.produtos.map(produto => {
                        return{
            ...
            case 2:
                if(this.servicos.length == 0) {
                    ...
                    let servMaisConsumido = this.servicos.map(servico => {
                        return{
    ...
    public listagem10MenosConsumoQnt(): void{
        ...
        switch(opcaoListTop10){
            case 1:
                ...
                }).sort((a, b) => a.totalProdConsumido - b.totalProdConsumido).slice(0, 10)
            ...
            case 2:
                ...
                }).sort((a, b) => a.totalServConsumido - b.totalServConsumido).slice(0, 10)
    ...
    public listagem5ConsumoValor(): void{
        ...
        function pegarValorTotal(produtos: Array<Produto>, servicos: Array<Servico>): number{
            let valorTotal = 0
        ...
        let clienteOrdenado = this.clientes.map(cliente => {
            return{
                nome: cliente.nome,
                cpf: cliente.getCPF,
                valorTotalConsumido: pegarValorTotal(cliente.getProdutosConsumidos, cliente.getServicosConsumidos)
            }
        }).sort((a, b) => b.valorTotalConsumido - a.valorTotalConsumido).slice(0, 5)
        ...
        clienteOrdenado.forEach(cliente => {
            ...
            console.log(`     Valor total de Consumos: ${cliente.valorTotalConsumido}`)
        })
    }
}